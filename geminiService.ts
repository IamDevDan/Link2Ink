...
// Fallback API Key provided by the user to ensure runtime availability
// even if Vercel env vars are not exposed to the client bundle correctly.
const USER_FALLBACK_KEY = "AIzaSyDlbc0ghJDzpUc-8zW5FVdjNGK9jHn1NIM";

const getApiKey = (): string => {
  try {
    // 1. Try process.env (Node/Standard Bundlers)
...
export async function generateInfographic(
  ...
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash-image', // Switched to Flash Image for better availability
      contents: {
        parts: [{ text: prompt }],
      },
      config: {
...
export async function generateArticleInfographic(
  ...
  language: string = "English"
    ...
    try {
        ...
        // Switched to 'gemini-2.5-flash' for research phase.
